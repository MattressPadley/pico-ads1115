...
struct ADCReading {
    ...
    ADCReading(int16_t raw, float volt, ADCChannel ch, uint32_t time = 0)
        : raw_value(raw), voltage(volt), channel(ch), timestamp(time), valid(true) {}
};
...
constexpr float getVoltageRange(GainAmplifier gain) {
    switch (gain) {
        case GainAmplifier::GAIN_TWOTHIRDS: return 6.144f;
        case GainAmplifier::GAIN_ONE: return 4.096f;
        case GainAmplifier::GAIN_TWO: return 2.048f;
        case GainAmplifier::GAIN_FOUR: return 1.024f;
        case GainAmplifier::GAIN_EIGHT: return 0.512f;
        case GainAmplifier::GAIN_SIXTEEN: return 0.256f;
...
constexpr uint16_t getDataRateHz(DataRate rate) {
    switch (rate) {
        case DataRate::SPS_8: return 8;
        case DataRate::SPS_16: return 16;
        case DataRate::SPS_32: return 32;
        case DataRate::SPS_64: return 64;
        case DataRate::SPS_128: return 128;
        case DataRate::SPS_250: return 250;
        case DataRate::SPS_475: return 475;
        case DataRate::SPS_860: return 860;
        default: return 128;
    }
}
...
constexpr bool isValidChannel(ADCChannel channel) {
    return static_cast<uint8_t>(channel) <= static_cast<uint8_t>(ADCChannel::DIFFERENTIAL_2_3);
}
...
constexpr bool isDifferentialChannel(ADCChannel channel) {
    return static_cast<uint8_t>(channel) >= static_cast<uint8_t>(ADCChannel::DIFFERENTIAL_0_1);
}
...
constexpr int16_t voltageToRaw(float voltage, GainAmplifier gain) {
    ...
    if (raw_float > 32767.0f) return 32767;
